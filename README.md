...
In order to have a the application working you have to follow these steps after dowloading this repo:
...
You can use the app running the server and doing some request to the available endpoint:

```
rails s
```

I have interpreted from the instructions that the mandatory params are the week number and the year

And you can use also the optional param merchant_id

Something like:
```
0.0.0.0:3000/disbursements?merchant=1&week_number=43&year=2018
```

To execute the job you can use the rails console:
```
rails c
GenerateDisbursementsJob.perform_now
```
...
-Add some tests for the controller and other points

-Refactor controller and scopes in a more efficient way
...
-Allow the job to receive parameters to customize what orders should be disbursed
